...
def process_dhcp_packet(packet): # definition de la fonction process_dhcp_packet d'argument packet
    if packet.haslayer(scapy.DHCP):
        ...
        dhcp_info = DhcpPacketInfo(trame_type, source_ip, destination_ip, source_mac, destination_mac, lease_time, subnet_mask, routeur, domain_name, dns_ip) # renvoie de l'objet dhcp_info pour la récupération sur la bdd
        print(dhcp_info.trame_type)
        return dhcp_info
